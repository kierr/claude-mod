...
function createInstaller({ home, cacheDir, validateArtifact, ensureSharpBindings,
  ...
  const root = path.join(home, ".local", "lib", "claude-mods");
  ...
  function assertLauncher() {
    ...
    if (target && target !== launcherTarget) {
  ...
  function install(version) {
    ...
    try {
      ...
      if (statOrNull(cacheBuild)) {
        ...
        verify(path.join(staging, "deobfuscated.js"), version);
      ...
      verify(path.join(candidate, "deobfuscated.js"), version);
      assertLauncher();
      fs.mkdirSync(path.dirname(launcher), { recursive: true });
      if (!statOrNull(launcher)) {
        fs.symlinkSync(launcherTarget, launcher);
        newLauncher = true;
      }
      ...
      if (newLauncher && linkTarget(launcher) === launcherTarget) fs.unlinkSync(launcher);
      throw error;
    } finally {
  ...
  function uninstall() {
    const target = linkTarget(launcher);
    if (target && target !== launcherTarget && !legacyTargets.includes(target)) {
