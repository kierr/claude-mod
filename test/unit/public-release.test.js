...
describe('public release contracts', () => {
  ...
  test('the CLI rename preserves existing on-disk state and launcher ownership', () => {
    expect(read('bin/patch.cjs')).toContain('path.join(os.homedir(), ".cache", "claude-mod")');
    expect(read('lib/installation.cjs')).toContain('path.join(home, ".local", "lib", "claude-mod")');
    expect(read('bin/discover-env-vars.cjs')).toContain('".cache", "claude-mod"');
    expect(read('README.md')).not.toContain('claude-mods');
  });
